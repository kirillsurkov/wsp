...
void physics_oimo_t::add_body(const physics_object_t::body_t& body) {
    rapidjson::StringBuffer buffer;
    rapidjson::Writer<rapidjson::StringBuffer> writer(buffer);
    writer.StartObject();
    ...
    for (const auto& shape : body.get_shapes()) {
        switch (shape->type) {
            case physics_object_t::shape_type::sphere: {
                writer.String("sphere");
                break;
            }
            case physics_object_t::shape_type::box: {
                writer.String("box");
                break;
            }
            case physics_object_t::shape_type::cylinder: {
